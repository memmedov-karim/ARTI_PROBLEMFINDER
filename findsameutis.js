...
const {findUtisFromLine,findNameFromLine,findSurnameFromLine} = require('./helper.js')
...
function detecNumOfName(name,sur,txtFile){
  ...
  let numOfUtis = 0;
  for(let i of data){
    if(name === findNameFromLine(i) && sur===findSurnameFromLine(i)){
      numOfUtis+=1;
    }
  }
  return numOfUtis
...
detectSameUtis("Cəlalə ümumi.txt")